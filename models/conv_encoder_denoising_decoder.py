...
class ConvEncoderDenoisingDecoder(SeparatorModel):
    ...
    def encoder(self, input_tensor):
        ...
        # 3rd downsampling
        conv3 = self.conv_block(downsample1, 64, self.kernel_size)
        downsample3 = self.conv_block(conv3, 64, self.kernel_size, strides=(2, 2))
        # intermediate low dimensional features
        latent = self.conv_block(downsample3, 128, self.kernel_size)
        latent = self.conv_block(latent, 128, self.kernel_size)
        return latent, conv3, conv2, conv1
    ...
    def decoder(self, input_tensors, name=None):
        ...
        x = self.conv_block(util.crop_and_concat(upsample1, conv3), 32, self.kernel_size)
        # 2nd upsampling
        upsample2 = keras.layers.UpSampling2D((2, 2))(x)
        x = self.conv_block(util.crop_and_concat(upsample2, conv2), 16, self.kernel_size)
        # 3rd upsampling
        upsample3 = keras.layers.UpSampling2D((2, 2))(x)
        x = self.conv_block(util.crop_and_concat(upsample3, conv1), 8, self.kernel_size)
