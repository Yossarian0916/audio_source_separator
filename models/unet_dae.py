...
class UnetAutoencoder:
    ...
    def autoencoder(self, input_tensor, kernel_size, name=None):
        """denoising autoencoder to further remove noise"""
        # downsampling
        x = self.conv1d(input_tensor, self.frames, kernel_size, 2)
        x = self.conv1d(x, self.frames, kernel_size, 2)
        # upsampling
        x = keras.layers.UpSampling1D(2)(x)
        x = self.conv1d(x, self.frames, kernel_size)
        x = keras.layers.UpSampling1D(2)(x)
        x = self.conv1d(x, self.frames, kernel_size)
        ...
        output = self.conv1d(x, self.frames, kernel_size, name=name)
        return output
    ...
    def get_model(self, name='unet_dae_separator'):
        ...
        vocals = self.autoencoder(vocals_input, 11, name='vocals')
        bass = self.autoencoder(bass_input, 11, name='bass')
