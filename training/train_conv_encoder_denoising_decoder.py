...
log_dir = os.path.join(training_path, "./logs/conv_encoder_denoising_decoder/" + datetime.now().strftime("%Y%m%d_%H%M"))
callbacks = [
...
saved_model_name = os.path.join(saved_model_dir, 'conv_encoder_denoising_decoder?time={}.h5'.format(date_time))
model.save(saved_model_name)
print("\nModel Saved Successful!")
saved_weight_dir = os.path.join(root, 'saved_model', 'weight_checkpoints')
saved_weights_name = os.path.join(saved_weight_dir, 'conv_encoder_denoising_decoder?time={}.h5'.format(date_time))
